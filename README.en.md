<!-- Keep this file semantically aligned with README.md. See CLAUDE.md. -->
...
| Native Windows shells | cmd, PowerShell, and Git Bash are not adapted or verified |
...
Plugin installation places a plugin-scoped copy of `deaddrop` inside that agent's plugin; it does **not** install a global `deaddrop` command in your shell.
...
For the complete two-session flow, including the hook output and the final pasted prompt, see the [usage example](docs/USAGE.en.md).
...
Decisions and external research live in [ADR.md](ADR.md), and [Adding an agent](docs/ADDING-AN-AGENT.md) is the end-to-end adapter and packaging guide. To report a problem, [open an issue](https://github.com/powtick/agent-deaddrop/issues) with the agent version, operating system, and observed hook result.

To contribute a change:

1. **Align on the design first.** Read the relevant [DESIGN.md](DESIGN.md) sections and the module you plan to touch, along with its existing tests. For anything beyond a small fix, open an issue to agree on the approach; if a requirement and DESIGN.md disagree, resolve that before writing code.
2. **Work on a branch and keep each change focused.** Fork first if you do not have write access.
3. **Keep the user docs bilingual and in sync.** Any user-visible change must update `README.md` and `README.en.md`—and, when relevant, `docs/USAGE.md` and `docs/USAGE.en.md`—in the same commit, with equivalent facts, commands, and options in both languages. Internal docs such as `DESIGN.md` stay Chinese.
4. **Ship tests with the behavior.** Extraction changes need a fixture plus its expected output under `tests/`; `drop`, `pickup`, and `hook-prompt` changes need matching assertions. The suite auto-discovers every adapter under `adapters/`.
5. **Bump `VERSION` for packaging changes.** When the current `VERSION` already has a matching `v<VERSION>` tag, any change under `bin/`, `adapters/`, `packaging/`, or `scripts/package-plugins.sh` must increment the root `VERSION` (semver); an untagged release candidate may keep the same version. Never hand-edit manifest versions—packaging injects them from `VERSION`.
6. **Pass the full gate before pushing.** Run `tests/run.sh` and the static checks below; CI re-runs them on Ubuntu and macOS (macOS covers Bash 3.2), and a pull request must be green to merge.
...
Releases are tag-driven: `main` holds only canonical source and packaging templates, and pushing a `v<VERSION>` tag whose commit is on `origin/main` publishes the self-contained `marketplace` branch through CI. Do not commit generated `plugins/` or `.dist/` trees.
